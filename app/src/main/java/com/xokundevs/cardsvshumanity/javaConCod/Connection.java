...
public class Connection {
    ...
    public static final int INVALID_CREDENTIALS_ERROR = -4;
    ...
    /*public static class ConnectionThread extends Thread {
        ...
        public ConnectionThread(int command, @NonNull Activity activityContext, Object... arguments) {

            if(activityContext instanceof OnConnectionListener) {
                this.mListener = (OnConnectionListener) activityContext;
            }
            else {
                throw new RuntimeException("The activity must implement OnConnectionListener");
            }
        ...
        private void executeCommand() {
            switch (command) {
                ...
                case GET_BASIC_INFO_BARAJA:
                    getRunGetInfoBasicBaraja();
                    break;

                case GET_CARTAS_BARAJA:
                    getRunGetCartasFromBaraja();
                    break;
                ...
                case COGER_PARTIDA:
                    getRunPartidas(); // TODO Coger partida
                    break;
                ...
                case CONNECTAR_PARTIDA:
                    getRunConectarPartida();
                    break;
                default:
                    throw new IllegalArgumentException(command + " is not an expected value");
            }
        }

        private void getRunPartidas() {
            Socket sk = null;
            try {
                ...
                try {
                    ...
                    onConnectionFailure(SOCKET_DISCONNECTED);
                ...
                onConnectionSuccess(listPartidas);

            } catch (IOException e) {
                onConnectionFailure(UNKOWN_ERROR);
                e.printStackTrace();
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        ...
        private void getRunEraseUser() {
            ...
            try {
                ...
                if (result == OK) {
                    ...
                    onConnectionFailure(skHandler.recibirInt(secretKey));
                }
            } catch (IOException e) {
                e.printStackTrace();
                onConnectionFailure(UNKOWN_ERROR);
        ...
        private void getRunGetInfoBasicBaraja() {
            Socket sk = null;
            try {
                if (activityContext == null)
                    throw new Exception("No context added");
                ...
                if (user == null) {
                    onConnectionFailure(USER_NOT_LOGINED);
        ...
        private void getRunGetCartasFromBaraja() {
            Socket sk = null;
            try {
                if (activityContext == null)
                    throw new Exception("No context added");
        ...
        private void getRunBorraBaraja() {
            Socket sk = null;
            try {
                if (activityContext == null)
                    throw new Exception("No context added");
        ...
        private void getRunCrearPartida() {
            Socket sk = null;
            try {
                if (activityContext == null)
                    throw new Exception("No context added");
                ...
                if (result == OK) {
                    ...
                    if (result == OK) {
                        onConnectionSuccess(new Object[]{skHandler, secretKey});
                    } else {
                        onConnectionFailure(error);
                    }
                } else {
        ...
        private void getRunConectarPartida() {
            Socket sk = null;
            try {
                if (activityContext == null)
                    throw new Exception("No context added");
        ...
        public interface OnConnectionListener {
            void onConnectionSuccess(Object result);

            void onConnectionError(int error);

            void onConnectionStart();

            void onConnectionFinishes();
        }

        private void onConnectionSuccess(final Object data) {
            activityContext.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    mListener.onConnectionSuccess(data);
                }
            });
        }


        private void onConnectionFailure(final int errorCode) {
            activityContext.runOnUiThread(new Runnable() {
                @Override
                public void run() {
        ...
        private void onConnectionEnd() {
            activityContext.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    mListener.onConnectionFinishes();
                }
            });
        }
